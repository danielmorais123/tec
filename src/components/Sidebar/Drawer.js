...
const Drawer = ({ open, setOpen }) => {
    return (
        <motion.div initial={{ x: -200, opacity: 0 }}
            ...
            transition={{ duration: 0.5 }} id="drawer-navigation" className={`xl:hidden   xs:rounded-tr-[30px] ${!open && "hidden"} fixed z-40 h-screen p-4 overflow-y-auto bg-purple-300 w-full xs:w-80 dark:bg-gray-800`} tabindex="-1" aria-labelledby="drawer-navigation-label">
            <h5 id="drawer-navigation-label" className="text-base font-semibold text-gray-500 uppercase dark:text-gray-400">Menu</h5>
            <button onClick={() => setOpen(!open)} data-drawer-dismiss="drawer-navigation" aria-controls="drawer-navigation" className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 absolute top-2.5 right-2.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" >
